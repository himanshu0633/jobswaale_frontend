...
import { getNextMasterId } from '../utils/masterForm';
import { 
  Plus, 
  Edit2, 
  X, 
...
export const Country = () => {
  ...
  // Form State
  const [form, setForm] = useState({ cid: '', countryName: '', status: 'active' });
  ...
  const getNextId = async () => {
    try {
      return await getNextMasterId(axios, `${BASE_API_URL}/masters/countries`, 'cid');
    } catch (err) {
      console.error(err);
      return '';
    }
  ...
  const fetchList = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_API_URL}/masters/countries?page=${currentPage}&limit=${entriesPerPage}&search=${search}&paginate=true`
      );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.cid) {
      showAlert('error', 'Country ID is required.');
      return;
    }
    if (!form.countryName) {
    ...
    try {
      if (editingId) {
        // Edit Mode
        await axios.put(`${BASE_API_URL}/masters/countries/${editingId}`, {
        ...
        setTimeout(() => {
          setView('list');
          setEditingId(null);
          setForm({ cid: '', countryName: '', status: 'active' });
          fetchList(); // reload table
        }, 1500);
      } else {
        // Add Mode
        await axios.post(`${BASE_API_URL}/masters/countries`, {
          cid: form.cid,
          countryName: form.countryName,
          status: form.status
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this country?")) {
      try {
        ...
        console.error(err);
        showAlert('error', err.response?.data?.message || 'Error deleting country.');
      }
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 text-[0.9rem]">
          ...
          <span className="text-indigo-600">
            {view === 'list' ? 'Manage Country' : editingId ? 'Edit Country' : 'Add Country'}
          </span>
        </div>
      </div>
          ...
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            ...
            <button
              onClick={async () => {
                ...
                setForm({ cid: nextId, countryName: '', status: 'active' });
                setAlert({ type: '', text: '' });
                setView('form');
              }}
          ...
          <ResponsiveCardList
            ...
            renderCard={(item, index) => (
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-bold text-slate-800">{item.countryName}</div>
                    <div className="text-xs text-slate-500">ID: {String(item.cid).padStart(3, '0')}</div>
                  </div>
          ...
          <div className="overflow-x-auto relative min-h-[200px] hidden md:block">
            ...
            <table className="w-full text-xs md:text-sm text-left border-collapse min-w-[640px]">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {list.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-6 py-8 text-center text-slate-400">No matching records found.</td>
                  </tr>
                ) : (
                  list.map((item, index) => (
                      <tr
                      ...
                      <td className="px-6 py-4 font-medium text-slate-700">{item.countryName}</td>
