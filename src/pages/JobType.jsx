...
export const JobType = () => {
  ...
  const [form, setForm] = useState({ id: '', jobType: '', sortingNo: 0, status: 'active' });
  ...
  const fetchList = async () => {
    try {
      ...
      console.error(err);
      showMessage('error', 'Error retrieving job types.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.id || !form.jobType) return;

    try {
      if (editingId) {
        const response = await axios.put(`${BASE_API_URL}/masters/job-types/${editingId}`, form);
        setList(list.map(item => item._id === editingId ? response.data : item));
        showMessage('success', 'Job type updated successfully.');
        setEditingId(null);
      } else {
        const response = await axios.post(`${BASE_API_URL}/masters/job-types`, form);
        setList([...list, response.data]);
        showMessage('success', 'Job type added successfully.');
      }
      setForm({ id: '', jobType: '', sortingNo: 0, status: 'active' });
    } catch (err) {
  ...
  const handleEdit = (item) => {
    ...
    setForm({ id: item.id, jobType: item.jobType, sortingNo: item.sortingNo, status: item.status });
  };

  const handleDelete = async (uid) => {
    if (!window.confirm('Delete this job type?')) return;
    try {
      await axios.delete(`${BASE_API_URL}/masters/job-types/${uid}`);
      setList(list.filter(item => item._id !== uid));
      showMessage('success', 'Job type deleted successfully.');
    } catch (err) {
      showMessage('error', 'Error deleting job type.');
    }
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Configure default job types (e.g. Full Time, Part Time, Contract, Internship).
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                  Job Type ID (Unique Code)
                </label>
                <input
                  type="text"
                  required
                  disabled={!!editingId}
                  placeholder="e.g. FT, PT, INT"
                  value={form.id}
        ...
        <div className="lg:col-span-2 border border-slate-200 bg-white rounded-2xl shadow-sm">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50 rounded-t-2xl">
            <h3 className="text-lg font-bold text-slate-800">Job Type Listings</h3>
          </div>
            ...
            <table className="w-full text-sm text-left text-slate-500">
              <thead className="text-xs uppercase bg-slate-50 text-slate-400">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold">Job Type Name</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {list.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-4 py-8 text-center text-slate-400">No job types defined yet.</td>
                  </tr>
                ) : (
                  list.map((item) => (
                    <tr key={item._id} className="hover:bg-slate-50/50">
                      <td className="px-4 py-3 font-bold text-slate-800 uppercase">{item.id}</td>
                      <td className="px-4 py-3 font-medium text-slate-700">{item.jobType}</td>
