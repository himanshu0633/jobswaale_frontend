...
export const Qualification = () => {
  ...
  const [form, setForm] = useState({ id: '', name: '', sortingNo: 0, status: 'active' });
  ...
  const fetchList = async () => {
    try {
      ...
      console.error(err);
      showMessage('error', 'Error retrieving qualifications.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        const response = await axios.put(`${BASE_API_URL}/masters/qualifications/${editingId}`, form);
        setList(list.map(item => item._id === editingId ? response.data : item));
        showMessage('success', 'Qualification updated successfully.');
        setEditingId(null);
      } else {
        const response = await axios.post(`${BASE_API_URL}/masters/qualifications`, form);
        setList([...list, response.data]);
        showMessage('success', 'Qualification added successfully.');
      }
      setForm({ id: '', name: '', sortingNo: 0, status: 'active' });
    } catch (err) {
  ...
  const handleEdit = (item) => {
    ...
    setForm({ id: item.id, name: item.name, sortingNo: item.sortingNo, status: item.status });
  };

  const handleDelete = async (uid) => {
    if (!window.confirm('Delete this qualification?')) return;
    try {
      await axios.delete(`${BASE_API_URL}/masters/qualifications/${uid}`);
      setList(list.filter(item => item._id !== uid));
      showMessage('success', 'Qualification deleted successfully.');
    } catch (err) {
      showMessage('error', 'Error deleting qualification.');
    }
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Configure default qualification options for jobseeker profiles.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 border border-slate-200 bg-white rounded-2xl shadow-sm">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50 rounded-t-2xl">
            <h3 className="text-lg font-bold text-slate-800">Qualification Listings</h3>
          </div>
            ...
            <table className="w-full text-sm text-left text-slate-500">
              <thead className="text-xs uppercase bg-slate-50 text-slate-400">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold">Qualification Name</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {list.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-4 py-8 text-center text-slate-400">No qualifications defined yet.</td>
                  </tr>
                ) : (
                  list.map((item) => (
