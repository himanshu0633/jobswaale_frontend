...
export const State = () => {
  ...
  // Form State
  const [form, setForm] = useState({ cid: '', sid: '', stateName: '', status: 'active' });
  ...
  // Fetch initial countries dropdown list
  const fetchCountries = async () => {
    try {
      const response = await axios.get(`${BASE_API_URL}/masters/countries`);
      setCountries(sortByText(response.data, 'countryName'));
    } catch (err) {
      console.error(err);
      showAlert('error', 'Error retrieving countries.');
    }
  };

  const getNextId = async () => {
    try {
      return await getNextMasterId(axios, `${BASE_API_URL}/masters/states`, 'sid');
    } catch (err) {
      console.error(err);
      return '';
    }
  ...
  const fetchList = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_API_URL}/masters/states?page=${currentPage}&limit=${entriesPerPage}&search=${search}&paginate=true`
      );
      ...
      console.error(err);
      showAlert('error', 'Error retrieving states.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { cid, sid, stateName, status } = form;
    if (!cid) {
      showAlert('error', 'Country selection is required.');
      return;
    }
    if (!sid) {
      showAlert('error', 'State ID is required.');
      return;
    }
    if (!stateName) {
      showAlert('error', 'State Name is required.');
      return;
    }
    ...
    try {
      if (editingId) {
        // Edit Mode
        await axios.put(`${BASE_API_URL}/masters/states/${editingId}`, {
          cid,
          stateName,
          status
        });
        ...
        setTimeout(() => {
          setView('list');
          setEditingId(null);
          setForm({ cid: '', sid: '', stateName: '', status: 'active' });
          fetchList(); // reload table
        }, 1500);
      } else {
        // Add Mode
        await axios.post(`${BASE_API_URL}/masters/states`, {
          cid,
          sid,
          stateName,
          status
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this state?")) {
      try {
        ...
        console.error(err);
        showAlert('error', err.response?.data?.message || 'Error deleting state.');
      }
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 text-[0.9rem]">
          ...
          <span className="text-indigo-600">
            {view === 'list' ? 'Manage State' : editingId ? 'Edit State' : 'Add State'}
          </span>
        </div>
      </div>
          ...
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            ...
            <button
              onClick={async () => {
                ...
                setForm({ cid: '', sid: nextId, stateName: '', status: 'active' });
                setAlert({ type: '', text: '' });
                setView('form');
              }}
          ...
          <ResponsiveCardList
            ...
            renderCard={(item, index) => (
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-bold text-slate-800">{item.stateName}</div>
                    <div className="text-xs text-slate-500">ID: {String(item.sid).padStart(3, '0')}</div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-xs text-slate-500">Country: {item.country?.countryName || 'N/A'} • Sort: {item.sortingNo}</div>
                ...
                {list.length === 0 ? (
                  ...
                  list.map((item, index) => (
                      ...
                      <td className="px-6 py-4 text-slate-500">
                        {getCountryName(item.cid)}
                      </td>
                      <td className="px-6 py-4 font-medium text-slate-700">{item.stateName}</td>
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid gap-6 md:grid-cols-3">
                ...
                <input type="hidden" value={form.sid} readOnly />
