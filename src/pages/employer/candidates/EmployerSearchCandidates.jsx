              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? <tr><td colSpan="6" className="px-5 py-12 text-center"><Loader className="mx-auto h-7 w-7 animate-spin text-[#6658dd]" /></td></tr> : data.candidates.length ? data.candidates.map((candidate) => (
                  <tr key={candidate.id} className="transition hover:bg-slate-50">
                    ...
                    <td className="relative px-5 py-4 text-center"><button type="button" onClick={() => setOpenMenuId(openMenuId === candidate.id ? null : candidate.id)} className="rounded p-2 text-slate-400 transition hover:bg-slate-100 hover:text-[#6658dd]" aria-label={`Actions for ${candidate.name}`}><MoreVertical className="h-4 w-4" /></button>{openMenuId === candidate.id && <><button type="button" className="fixed inset-0 z-10 cursor-default" onClick={() => setOpenMenuId(null)} aria-label="Close menu" /><div className="absolute right-6 top-12 z-20 w-44 rounded-md border border-slate-100 bg-white py-1.5 text-left shadow-lg"><Link to="/employer/candidates" className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50"><User className="h-4 w-4" /> View Profile</Link><Link to="/employer/messages" className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50"><MessageCircle className="h-4 w-4" /> Contact</Link>{candidate.resume ? <a href={candidate.resume} className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50"><Download className="h-4 w-4" /> Download Resume</a> : <button type="button" className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs font-bold text-slate-400"><Download className="h-4 w-4" /> No Resume</button>}</div></>}</td>
                  </tr>
