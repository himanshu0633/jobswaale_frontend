  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        <div>
          <h1 className="text-2xl font-extrabold text-[#3f4254]">Talent Pool</h1>
          <p className="mt-1 text-sm font-semibold text-slate-400">Save, organize, and manage candidate profiles for future vacancies.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-5">
        {[
          ...
          <div key={idx} className="rounded-lg border border-slate-100 bg-white p-5 shadow-sm flex items-center gap-3.5">
            <span className={`flex h-12 w-12 items-center justify-center rounded-full shrink-0 ${item.color}`}>
              <item.icon className="h-5.5 w-5.5" />
            </span>
            <div>
              <p className="text-xs font-bold text-slate-400">{item.title}</p>
              <p className="mt-0.5 text-xl font-black text-[#3f4254]">{item.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-lg border border-slate-100 bg-white shadow-sm overflow-hidden">
        {/* Card Header */}
        <div className="border-b border-slate-100 p-5 bg-slate-50/50 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className="inline-flex h-10 items-center justify-center gap-1.5 rounded-lg bg-[#6658dd] px-4.5 text-xs font-extrabold text-white transition hover:bg-[#5848d8] shadow-sm"
            >
            ...
            <a
              ...
              className="inline-flex h-10 items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-4.5 text-xs font-extrabold text-slate-600 transition hover:bg-slate-50"
            >
        ...
        <div className="p-5 space-y-4">
          ...
          {loading ? (
            ...
            <div className="text-center py-10 border border-dashed border-slate-100 rounded-lg space-y-2">
              ...
              <p className="text-xs font-semibold text-slate-400 max-w-sm mx-auto">Try refining your filter preferences or add new candidate profiles to your bookmarks list.</p>
            </div>
          ) : (
                    ...
                    <tr key={candidate.talentPoolId} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <td className="py-4.5 px-4 text-center relative">
                        <div className="inline-block text-left">
                          <button
                            onClick={() => setActiveDropdownId(activeDropdownId === candidate.talentPoolId ? null : candidate.talentPoolId)}
                          ...
                          {/* Dropdown Menu */}
                          {activeDropdownId === candidate.talentPoolId && (
                            <>
                              <div className="fixed inset-0 z-10" onClick={() => setActiveDropdownId(null)}></div>
                              <div className="absolute right-4 mt-1 w-40 rounded bg-white shadow-lg border border-slate-100 z-20 text-left py-1 text-xs">
                                <a
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    handleRemoveCandidate(candidate.talentPoolId);
                                  }}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-lg rounded-lg bg-white shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <h3 className="font-black text-lg text-[#3f4254] flex items-center gap-1.5">
                <FolderPlus className="h-5.5 w-5.5 text-[#6658dd]" />
              ...
              {!selectedCandidate ? (
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="rounded bg-[#6658dd] px-4 py-2 font-extrabold text-white hover:bg-[#5848d8] transition flex items-center gap-1"
                    >
                    ...
                    {searchingNonPool ? (
                      ...
                      nonPoolCandidates.map(c => (
                        <div
                          ...
                          className="p-3 hover:bg-slate-50/60 transition cursor-pointer flex justify-between items-center"
                        >
                          <div className="flex items-center gap-2">
                            <div className="h-8 w-8 rounded-full bg-indigo-50 text-[#6658dd] flex items-center justify-center font-extrabold text-[10px]">
                              {c.initials}
                            </div>
                            <div>
                              <span className="font-extrabold text-[#3f4254] text-xs block">{c.name}</span>
                              <span className="text-[10px] font-semibold text-slate-400 block">{c.email} • {c.location}</span>
                            </div>
                          </div>
                          <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 border border-indigo-100 rounded px-1.5 py-0.5">
                            {c.experience}
                          </span>
                        </div>
                      ))
                ...
                <form onSubmit={handleAddToPoolSubmit} className="space-y-4">
                  <div className="rounded-lg bg-slate-50 p-4 border border-slate-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-[#6658dd] text-white font-black flex items-center justify-center">
                        {selectedCandidate.initials}
                      </div>
                      <div>
                        <h5 className="font-extrabold text-sm text-[#3f4254]">{selectedCandidate.name}</h5>
                        <p className="text-xs font-semibold text-slate-400">{selectedCandidate.email} • {selectedCandidate.location}</p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setSelectedCandidate(null)}
                      className="text-xs font-black text-rose-600 bg-rose-50 px-2.5 py-1 rounded hover:bg-rose-100 transition"
                    >
                      Change
                    </button>
                  </div>
