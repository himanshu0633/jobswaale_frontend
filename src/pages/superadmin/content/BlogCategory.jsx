...
export const BlogCategory = () => {
  ...
  const getNextId = async () => {
    try {
      return await getNextMasterId(axios, `${BASE_API_URL}/cms/blog-categories`, 'id');
    } catch (err) {
      console.error(err);
      return '';
    }
  ...
  const getNextSort = async () => {
    try {
      return await getNextSortNo(axios, `${BASE_API_URL}/cms/blog-categories`, 'sortingNo');
    } catch (err) {
      console.error(err);
      return '';
    }
  ...
  const fetchList = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_API_URL}/cms/blog-categories?page=${currentPage}&limit=${entriesPerPage}&search=${search}&paginate=true`
      );
      ...
      console.error(err);
      showAlert('error', 'Error retrieving blog categories.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        // Edit Mode
        await axios.put(`${BASE_API_URL}/cms/blog-categories/${editingId}`, {
          name: form.name,
        ...
        // Add Mode
        await axios.post(`${BASE_API_URL}/cms/blog-categories`, {
          id: form.id,
          name: form.name,
          sortingNo: toWholeNumber(form.sortingNo),
          status: form.status
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this blog category?")) {
      try {
        await axios.delete(`${BASE_API_URL}/cms/blog-categories/${id}`);
        showAlert('success', 'Blog category deleted successfully.');
        fetchList();
      } catch (err) {
        console.error(err);
        showAlert('error', err.response?.data?.message || 'Error deleting blog category.');
      }
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 text-[0.9rem]">
          ...
          <span className="text-indigo-600">
            {view === 'list' ? 'Manage Blog Category' : editingId ? 'Edit Blog Category' : 'Add Blog Category'}
          </span>
        </div>
      </div>
            ...
            <div className="flex gap-2">
              <button
                onClick={async () => {
                  ...
                  setForm({ id: nextId, name: '', sortingNo: nextSort, status: 'active' });
                  setAlert({ type: '', text: '' });
                  setView('form');
                }}
