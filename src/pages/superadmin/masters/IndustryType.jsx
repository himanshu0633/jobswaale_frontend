...
export const IndustryType = () => {
  ...
  // Form State
  const [form, setForm] = useState({ id: '', industryType: '', sortingNo: '', status: 'active' });
  ...
  const fetchList = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_API_URL}/masters/industry-types?page=${currentPage}&limit=${entriesPerPage}&search=${search}&paginate=true`
      );
      ...
      console.error(err);
      showAlert('error', 'Error retrieving industry types.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 text-[0.9rem] ">
          ...
          <span className="text-indigo-600">
            {view === 'list' ? 'Manage Industry Type' : editingId ? 'Edit Industry Type' : 'Add Industry Type'}
          </span>
        </div>
      </div>
      ...
      {view === 'list' ? (
          ...
          <ResponsiveCardList
            ...
            renderCard={(item, index) => (
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-bold text-slate-800">{item.industryType}</div>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {list.length === 0 ? (
                  ...
                  list.map((item, index) => (
                      ...
                      <td className="px-6 py-4 font-medium text-slate-700">{item.industryType}</td>
        ...
        <div className="border border-slate-200 bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            <h3 className="text-base font-bold text-slate-800">
              {editingId ? 'Edit Industry Type' : 'Add Industry Type'}
            </h3>
            <button
              onClick={() => {
                setView('list');
