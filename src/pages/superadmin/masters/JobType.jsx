...
export const JobType = () => {
  ...
  // Form State
  const [form, setForm] = useState({ id: '', jobType: '', sortingNo: '', status: 'active' });
  ...
  const fetchList = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_API_URL}/masters/job-types?page=${currentPage}&limit=${entriesPerPage}&search=${search}&paginate=true`
      );
      ...
      console.error(err);
      showAlert('error', 'Error retrieving job types.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.jobType) {
      showAlert('error', 'Job Type Name is required.');
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        setTimeout(() => {
          setView('list');
          setEditingId(null);
          setForm({ id: '', jobType: '', sortingNo: '', status: 'active' });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this job type?")) {
      try {
        await axios.delete(`${BASE_API_URL}/masters/job-types/${id}`);
        showAlert('success', 'Job type deleted successfully.');
        fetchList();
      } catch (err) {
        console.error(err);
        showAlert('error', err.response?.data?.message || 'Error deleting job type.');
      }
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 text-[0.9rem]">
          ...
          <span className="text-indigo-600">
            {view === 'list' ? 'Manage Job Type' : editingId ? 'Edit Job Type' : 'Add Job Type'}
          </span>
        </div>
      </div>
          ...
          <ResponsiveCardList
            ...
            renderCard={(item, index) => (
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-bold text-slate-800">{item.jobType}</div>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {list.length === 0 ? (
                  ...
                  list.map((item, index) => (
                      ...
                      <td className="px-6 py-4 font-medium text-slate-700">{item.jobType}</td>
