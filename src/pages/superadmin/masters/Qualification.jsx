...
export const Qualification = () => {
  ...
  const getNextId = async () => {
    try {
      return await getNextMasterId(axios, `${BASE_API_URL}/masters/qualifications`, 'id');
    } catch (err) {
      console.error(err);
      return '';
    }
  ...
  const getNextSort = async () => {
    try {
      return await getNextSortNo(axios, `${BASE_API_URL}/masters/qualifications`, 'sortingNo');
    } catch (err) {
      console.error(err);
      return '';
    }
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this qualification?")) {
      try {
        await axios.delete(`${BASE_API_URL}/masters/qualifications/${id}`);
        showAlert('success', 'Qualification deleted successfully.');
        fetchList();
      } catch (err) {
        console.error(err);
        showAlert('error', err.response?.data?.message || 'Error deleting qualification.');
      }
    }
  };
